...
价格由使用者明确提供，不内置会过期的报价。调度器在发起整批请求前，用输入 UTF-8 字节上界加协议余量、输出上限及最大尝试次数预留预算。预留不回收，因此可能在实际账单远低于上限时提前停止；并发批次整体不满足预算时不会部分发起。服务端未提供用量时停止后续批次。报告同时保留预留预算和已知用量的估算费用。
