...
0.4.0 新运行按组件签名检查题目、接入/模型参数、预算与 CLI 身份；允许调整并发、进度间隔和输出目录，逐样本记录实际条件。旧版仍保留完整配置/源码校验，不能自动升级续跑。详见[兼容规则](compatibility.md)。它只补缺失的完成记录，已完成错误样本不重试。若进程在响应落盘前崩溃，该次可能已经被服务商计费，续跑无法保证远端 exactly-once；attempts 日志和保守预算保留这条证据边界。
